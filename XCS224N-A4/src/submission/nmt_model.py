...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        # print("dec initial hidden state shape: ", dec_init_state[0].shape)
        # print("dec initial cell state shape: ", dec_init_state[1].shape)
        enc_masks = self.generate_sent_masks(enc_hiddens, source_lengths)
        # print("enc_masks shape: ", enc_masks.shape)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        encoder_output, (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens, list_lengths = nn.utils.rnn.pad_packed_sequence(encoder_output, batch_first=True)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for y in torch.split(Y, split_size_or_sections = 1, dim = 0):
            Y_t = torch.squeeze(y)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden_T = torch.unsqueeze(dec_hidden, 2) # shape = (b, h, 1)
        e_t = torch.squeeze(torch.bmm(enc_hiddens_proj, dec_hidden_T), dim = 2)
        ### END CODE HERE
        ...
        alpha_t_reshape = torch.unsqueeze(alpha_t, 1)  # shape (b, 1, src_len)
        a_t = torch.squeeze(torch.bmm(alpha_t_reshape, enc_hiddens), dim = 1)
