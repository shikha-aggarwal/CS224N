...
class VocabEntry(object):
        ...
        for s in sents:
            ...
            for w in s:
                word_id = [self.start_of_word] + \
                    ...
                    [self.end_of_word]
                words_in_sen.append(word_id)
            word_ids.append(words_in_sen)
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        words2charidx = self.words2charindices(sents)
        padded_sents = pad_sents_char(words2charidx, self.char2id['<pad>'])
